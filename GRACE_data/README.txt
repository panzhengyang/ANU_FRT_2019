...
run the python pre_processing.py script which will extract and prepare the files for next steps i.e. deformation calculation.

The NGL_station_data.csv file is copied here and the first line which contains the header is deleted manually
The NGL_station_data.csv file has the info about the lat lon at which the GRACE elastic deformation time series is required. 
...
awk 'NR==FNR {a[$1];next}!($1 in a)' NGL_station_data.csv MIT_station_data.txt > additional_MIT_station.txt
